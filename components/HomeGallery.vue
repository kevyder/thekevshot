  ...
  <div id="gallery">
    <stack :monitor-images-loaded="true" :column-min-width="450" :gutter-width="10" :gutter-height="10">
      <stack-item v-for="image of images" :key="image.slug">
        <nuxt-img preset="gallery" v-b-modal="image.slug" preload :src="image.photograph" :alt="`${image.title} by thekevshot`" class="gallery-image" loading="lazy" sizes="sm:450px md:576px lg:768px xl:450px" />
        <b-modal :id="image.slug" size="xl" hide-footer >
          <nuxt-img class="modal-image" v-b-modal="image.slug" preload :src="image.photograph" :alt="`${image.title} by thekevshot`" loading="lazy" sizes="sm:100vw" placeholder />
        </b-modal>
      </stack-item>
    </stack>
  </div>
